...
dogs.forEach(
  each => (each.recommendedFood = Math.trunc(each.weight ** 0.75 * 28))
);
dogs.find(each => (each.owners.includes('Sarah') ? console.log(each) : ''));

const ownersEatTooMuch = dogs.filter(
  each => each.curFood > each.recommendedFood * 1.1
);
const ownersEatTooLittle = dogs.filter(
  each => each.curFood < each.recommendedFood * 0.9
);

console.log(
  `${ownersEatTooMuch
    .flatMap(each => each.owners)
...
const movUsd1 = movements.map(function (mov) {
  return mov * eurToUsd;
});

const movUsd2 = movements.map(mov => mov * eurToUsd);
console.log(movements);
console.log(movUsd1);

const a = movements.map((mov, i) => {
  if (mov > 0) return `${i + 1}: You deposited ${mov}`;
  else return `${i + 1}: You withdrew ${Math.abs(mov)}`;
});

console.log(a);

/*
const dogs = function (j, k) {
  const nJ = j.slice(1, -2);
  const both = [...nJ, ...k];
...
const curr = new Set(['USD', 'GBP', 'EUR']);
...
const calcAge = function (ages) {
  ...
  // const avg = sum.reduce((acc, age) => acc + age, 0) / sum.length;
